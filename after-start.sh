...
echo -n "import default kazoo sounds"
...
./sup kazoo_media_maintenance import_prompts /opt/kazoo-sounds/us en-us
docker exec -i --user root kazoo.$NETWORK rm -rf kazoo-sounds

echo -n "enable monster-ui applications"
docker cp monster-ui.$NETWORK:/usr/share/nginx/html/src/apps apps
docker cp apps kazoo.$NETWORK:/opt
rm -rf apps

./sup crossbar_maintenance init_apps /opt/apps http://kazoo.$NETWORK:8000/v2
docker exec -i --user root kazoo.$NETWORK rm -rf apps

echo -n "refresh kamailio dispatcher"
docker exec -i kamailio.$NETWORK kamcmd dispatcher.reload 

echo -n "commit couchdb to couchdb-init"
